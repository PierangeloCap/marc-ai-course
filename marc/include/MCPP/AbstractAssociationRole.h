#ifndef ASSOCIATIONROLE_H
#define ASSOCIATIONROLE_H
...
#define STAR std::numeric_limits<unsigned int>::max()
...
void AbstractAssociationRole<LinkType, MIN, MAX>::addLink(std::shared_ptr<LinkType> link) {
    auto links = this->countLinks() + 1;
    if (MAX < links) {
        throw MultiplicityConstraintException(links);
    }
